		...
		return function () {
			...
			if (Config.Party) {
				...
				if (myPartyId) for (let party = getParty(), acceptFirst; party && party.getNext();) party && typeof party === 'object' && (function () {
					...
					) && (print('INVITE -- ' + party.name) || true) && clickParty(party, BUTTON_INVITE_ACCEPT_CANCEL);// if player isn't invited, invite
					...
					) && (print('LEAVE PARTY -- ' + party.name) || true) && clickParty(party, BUTTON_LEAVE_PARTY);
				}).apply(party);
			}
			return true;
		}
