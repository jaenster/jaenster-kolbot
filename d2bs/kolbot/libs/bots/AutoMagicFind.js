...
(function (module, require) {
	...
	module.exports = function(...args) {
		try {
			running = true;
			AutoMagicFind.apply(AutoMagicFind,args);
		} finally {
			running = false;
		}
	};
	function AutoMagicFind(Config, Attack, Pickit, Pather, Town, Misc) {
		...
		// Before all the calculations happen, we want to do towning, and get merc.
		// As merc, can have an huge effect on the areas you can pwn (conviction)
		Town();
		...
		const inTownPromise = () => (new Promise(resolve => me.inTown && resolve() || !running && reject()).then(function () {
		...
		})).catch(_=>_); // Empty catch, just stopping this promise
			...
			try {
              ...
              const Diablo = require('../bots/Diablo');
							...
							[sdk.units.DiabloSealVizierInactive, sdk.units.DiabloSealVizierActive,
								...
								Diablo.openSeal(getUnit(2, seal));
							});
